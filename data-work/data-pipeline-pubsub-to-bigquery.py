...
# This data pipeline reads a PubSub message in JSON format and identifies which target table the data should write to based on "Category" key Identifier. 
# You can modify this based on use case to write to multiple destination tables in Bigquery.
# Also, this datapipeline uses a custom BigQuery function to process each element 
# and write Error messages to a Dead letter table with the Raw Message received from PubSub.
...
def run():
    ...
    # Set other options
    google_cloud_options.project = 'example-999999-j4'
    google_cloud_options.region = 'us-east4'
    google_cloud_options.job_name = 'dataflow-pipeline3'
    google_cloud_options.temp_location = 'gs://ingestion-bucket-999/temp'
    google_cloud_options.staging_location = 'gs://ingestion-bucket-999/staging'
    ...
    dead_letter_table_spec = 'example-999999-j4:local_dataset.dead_letter_table'

    with beam.Pipeline(options=options) as p:
        messages = (p 
                    | 'ReadFromPubSub' >> ReadFromPubSub(subscription='projects/example-999999-j4/subscriptions/Incoming_JSON_Sub')
                    | 'ParseMessages' >> beam.ParDo(ParseMessageFn())
                    # tag each output based on Key Identifier
        ...
        #for loop to process each tagged output
        for tag, table_spec in [('SaleInfo', 'example-999999-j4.local_dataset.sales'), ('OrderInfo', 'example-999999-j4.local_dataset.orders'),
                                ('ReturnInfo', 'example-999999-j4.local_dataset.returns')]:
                                global_window = "Window-" + tag
                                WriteToBq = "WriteTo" + tag
                                GlobalWindow_Failed = "GlobalWindow" + tag + "-Failed"
                                Write_To_dead_letter = "Error-" + tag
                                # write to bigquery table and tag any message that is failed to write as "failed"
                                messages_load = (
                                    messages[tag]
                                    | global_window >> beam.WindowInto(GlobalWindows())
                                    | WriteToBq >> beam.ParDo(WriteToBQ(table_spec)).with_outputs('failed')
                                    )
                                # write each failed message to dead letter table
                                messages_failed = (
